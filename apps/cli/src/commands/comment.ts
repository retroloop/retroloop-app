 ...
 *
 * **Three targets, because the store has three** (retro 4
 * `r-cli-review-thread-reply`). `--record --section` was the only one for a
 * session, so the owner's first review-level asks had no sanctioned answer and
 * the reply detoured through chat — the exact smuggling review-level threads were
 * built to end. `--thread <id>` answers a thread that already exists, review-level
 * or record-level, which is what "answer where he asked" needs; `--review` opens a
 * new one anchored to nothing.
 */
type CommentArgs = {
