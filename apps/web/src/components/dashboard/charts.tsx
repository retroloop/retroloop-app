 ...
 * being neglected, and a proportion meter besides. One cut kept the stack and the
 * nominal bars and removed the rest: the donut ("What The Open Queue Costs") and
 * the radar ("Which End Is Being Neglected") by name, and the other three with
 * the variations that housed them. They are deleted rather than left behind a
 * flag — an unreachable chart is code that has to keep compiling for nobody.
 *
 ...
 * caller, and the orientation split was their only reason to exist. The stack
 * keeps both orientations because `vertical` is still the right reading for a
 * sequence, and a component that supports one orientation today is one rewrite
 * away from supporting the other.
 *
...
export function LifecycleStack({
  ...
  return (
     ...
     * comparison of tick coordinates, which is a position assertion the browser
     * does half of for free and which would go green for the wrong reason the
     * day recharts changes its layout maths.
     */
    <div
        ...
        >
          ...
          {orientation === 'horizontal' ? (
            <>
               ...
               * the same orientation brought `Feature` into a gutter built for
               * four characters, and the axis cropped it to `eature`: text
