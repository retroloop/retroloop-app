...
/**
 * The retro's name and its identity line (KC-0020, N1/N3).
 *
...
const ROW: Row = {
  ...
  session: { id: 3, cwd: '/Users/haider/Developer/retro', startedAt: '2026-08-24T09:00:00.000Z' },
}
...
/**
 * **The fallback prints the GLOBAL id** (D5, the owner: *"untitled retros should
 * show the global id as fallback"*), and this fixture is what makes the assertion
 ...
 *
 * Still never a bare number — the directory stays, per ledger v2 #120 — and still
...
/**
 * The owner: *"On the home page it should show \"Session #X · Retro #Y\" instead
 * of something like: \"Retro #1 · Session 1\""*. Session first, because a
...
test('the identity line is the session, the ordinal and the directory', () => {
  expect(retroIdentityLine(ROW)).toBe('Session 3 · Retro #2 · /Users/haider/Developer/retro')
})
...
test('the directory is the last segment, whatever the path looks like', () => {
  expect(cwdBasename('/Users/haider/Developer/retro/')).toBe('retro')
  expect(cwdBasename('/Users/haider/Developer/retro///')).toBe('retro')
