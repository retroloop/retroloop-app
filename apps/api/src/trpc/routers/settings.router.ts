...
/**
 * The global settings — one key today, and it is OWNER RULING 2's guarantee:
 *
 ...
 * (KC-0004). Everything here would be true of a page that lied.
 *
...
export const settingsRouter = router({
   ...
   * (KC-0010), and a permission that could be granted by an omitted field would
   * be the worst possible place to start.
   *
   * The write appends a version rather than editing one, so the answer to *"has
   * it ever been on?"* survives every later change — which is what *"the user
   * can be certain"* asks for about the past as well as the present. The
