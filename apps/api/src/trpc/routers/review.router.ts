...
/**
 * **One act, and it is the human's** (retro 4 `r-one-finish-button`). There were
 * two — `finish` and `requestChanges`, one procedure per event name — and the
 * owner removed the second on sight: *"I don't like having two buttons… if I
 * request a lot of changes in comments and then I click finish review, I
 * shouldn't be able to do that. It should be clear from the content of the
 * comments rather than from a redundant button I can press wrong."*
 *
 * So this is what the page can do to a review, whole. `finish` says his side of
 * the round is closed; what happens next — the next revision, or the export —
 * is the AI's step, taken through the CLI (`retro review close`) after reading
 * what he wrote. There is no `close` procedure and there will not be one: the
 * tRPC context is unconditionally `human`, and closing is the AI's act.
 * Nothing here is ever inferred from silence, absence of comments, or time
 * passing (KC-0010).
 *
 ...
 *
 * Pressing it twice in a round is absorbed rather than refused (retro 4
 * `r-request-changes-multi-press`): the second call writes nothing and answers
 * exactly what the first one did.
 *
 * The input carries his optional final message on the round
