 ...
 * attributes procedures, plus `labels.unretire` and `attributes.unretire`,
 * plus `records.relate`.
 *
 ...
 *   retrospective", which is what the comments panel is. It began as
 ...
 *   inferred.
 * - `records.listAll` — the flat cross-retro records page. No
 ...
 *   page (the record page at `/records/:id`). Every other record
 ...
 * namespaces rather than fewer.** Labels and attributes are two pure
 * primitives — labels are just labels, attributes carry data, and composition
 * is the user's own convention and never a system mechanism — so
 ...
 * What is new is that the use case below accepts both — the AI marks
 ...
 *
 * **`requests.*` is gone, and it is not coming back**
 * (`r-remove-requests`). Three procedures shipped a second ask channel beside
 * the review's comment threads, and it was ruled out on first contact:
 * comments at the review level are enough, with one individual request per
 * comment. A comment is the one channel now. The
