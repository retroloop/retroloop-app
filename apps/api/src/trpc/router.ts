 ...
 * table in trpc.md, plus `retros.list`, the dashboard's read model (KC-0020,
 * shortlist N4), plus `threads.list` and `threads.resolve`, plus
 * `records.listAll`, `records.setLifecycle` and `records.byId`, plus the twelve
 * of session 10's labels and attributes, plus session 12's `labels.unretire`
 * and `attributes.unretire`, plus session 13's `records.relate`.
 *
 * **The number in that sentence was wrong for two sessions** — it said
 ...
 *   retrospective", which is what the comments panel is (session 7). It began as
 *   the read path review-level threads never had, since those hang off no record
 *   and no record-scoped call could reach them; it is now the only wire source
 *   for any thread, because `records.get` stopped carrying a record's threads
 *   when the inline rendering that read them went away.
 ...
 *   inferred.
 * - `records.listAll` — the flat cross-retro records page (session 8). No
 *   arrangement of the record calls above answers it: every one of them is
 *   scoped to one retrospective, and the whole point of the page is that it is
 *   not.
 ...
 *   page (session 9, the record page at `/records/:id`). Every other record
 *   read here takes `(retroId, rid)`, which is what addresses a record (A5) and
 *   is not something anybody types or bookmarks; this is the one that runs the
 *   global sequence backwards. Widening `records.get` instead would have put
 *   the identity line, the lifecycle and a whole timeline on every card of
 *   every review to serve one page — and `records.get` answers for a record
 *   *inside* a review, revision and all, which is the question this one is not
 *   asking.
 *
 * **The twelve of session 10 are the largest single widening this router has
 * taken, and the count is the argument for why they are three namespaces rather
 * than fewer.** The owner ruled labels and attributes into existence as two pure
 * primitives — *"labels are just labels"*, attributes carry data, and
 * *"composition is the USER'S convention … never a system mechanism"* — so
 * `labels.*` and `attributes.*` are separate namespaces rather than one
 * `definitions.*` that would be the first place a reader looked for the pairing
 * the system does not have. Each carries four definition procedures (list,
 * define, rename, retire) plus the one human write on a record.
 *
 * `settings.*` is the third, and it is two procedures because it is OWNER RULING
 * 2 rather than a preference: *"if it is disabled, the user can be certain that
 * the AI cannot mess around."* Nothing already here could carry it — a global
 * switch has no retrospective to hang off — and folding it into `labels.*`
 * would have made a guarantee that governs both primitives look like a property
 * of one.
 *
 * **What is deliberately not here**, on the every-procedure-is-a-thing-to-mock
 * rule: no `unretire` (the settings page is *"create, rename, retire"*, and a
 * fourth control waits for a retro to ask), no `retype` (a value already stored
 * was accepted under the old type), and no per-record read of either — a
 * record's labels ride on `records.get`, `records.byId` and `records.listAll`,
 * which the pages are already holding.
 *
 ...
 * half goes through the CLI in its own process like every other AI write
 * (KC-0004). What is new is that the use case below accepts both — the AI marks
 * what it fixed, the human marks from the browser.
 *
 * **`records.relate` is the second, and it is the AI's more than the human's**
 * (session 11: *"both actors can relate records, each relation carries
 * how-they-relate words, and the relation reads from both sides, so that AI can
 * easily find past records and build holistic solutions."*). It is one procedure
 * for two acts, on the standing `records.setLifecycle` set — *"the input widened
 * here and the procedure count did not move"* — with a boolean rather than a
 ...
 *
 * **`holds.set/clear` is gone** (retro 4 `r-remove-hold`). Hold left the verdict
 * axis in retro 3 and got two procedures of its own; seeing the control live,
 * the owner found the toggle unreadable and the concept already covered — *"I
 * can achieve the whole thing by selecting something to be only done with the
 * human in the loop. So we don't need the hold."* That is `involvement`, which
 * `decisions.record` already writes. The `holds` table keeps its rows.
 *
 * **`requests.*` is gone, and it is not coming back** (retro 4
 * `r-remove-requests`). Three procedures shipped a second ask channel beside
 * the review's comment threads, and the owner ruled on it the first time he saw
 * it: *"we can just have the comments at the review level. I can add one
 * individual request per comment."* A comment is the one channel now. The
 * request rows those procedures wrote stay in the store as history; nothing
 * here reaches them.
 */
