Feature: The core loop, end to end
  ...
  # The parking step in the middle of this loop was removed with
  # `r-remove-hold`, and so were the `held`/`holdNote` fields it put into the
  ...
  Scenario: the AI files a revision, the human reviews it, and the export is valid
    ...
    # The multi-solution shape, proved through the real stack: the CLI wrote it,
    # the server read it out of the blob, and this is the browser rendering it.
    # The layers below each prove their own half; this is the one place all of
    ...
    # `r-finish-button-reenables`: a finish review was reported as sent, but
    # after refreshing the page the Finish review button was enabled again.
    #
    ...
    # against a store that already holds it, which is what a browser refresh
    # does.
    #
    ...
    # database. So this reload is that same refresh, and the page has nothing
    # but the server to answer from.
    ...
    # left to address, and closes the review to export
    ...
    # The final-message channel, end to end: typed in the browser, written
