...
**Reading the pointers.** Three kinds of tag appear below and none of them is
required to follow the rule beside it — the substance is always restated inline,
so a pointer can be safely ignored on a first read.
...
and `L3` name layers: L1 is the database (SQLite triggers), L3 the use cases —
so "refused at L3 and backstopped at L1" means the rule is enforced in code and
the database would refuse it too. A **revision** is one complete AI-authored
draft of a retrospective's whole record set; a retrospective has one or more, and
a record changes only by appearing in a new one (`data-model.md` §Revision).
"The owner", "the human", "the user" and "the reviewer" are the same one person,
who is the product's only human actor.

Two rules run through all of it and are worth stating before the machines:

- **Nothing is inferred from silence** (KC-0010). No state is entered because a
...
or more, and **exactly one may be non-`finished` at a time** (KC-0011).
...
**`ReviewFinished` is an event, not a transition** (retro 4 `r-one-finish-button`).
The human's one button closes *his* side of a round and leaves the retrospective
`reviewing`. Between his finish and the AI's close he may still change a verdict,
undo one, or add a comment — that window is deliberate, and it is why the close
asks the finish gate again rather than trusting the answer the button gave.
...
event, no error (retro 4 `r-request-changes-multi-press`). That is the opposite
...
The owner, session 11 and carried on his ack at every close since: *"There should
be a status in between that indicates that the human has submitted but AI hasn't
closed"*. The window he means is the one the paragraphs above describe — after
`ReviewFinished`, before `ReviewClosed` — and until session 13 nothing said so:
the dashboard row and the review header both read REVIEWING for the whole of it,
and the only surface that knew was the review bar's Sent mark, at the bottom of
the page he had already left.
...
**The latest round is the one that counts.** A round he finished two revisions
ago was answered by the draft that followed it, and the retrospective is his
...
before keys off the same thing after. And the window stays writable: he may still
change a verdict, undo one, or comment — including undoing one back to `pending`,
...
1. **Retro 3 `r-hold-semantics`** took `hold` *off the verdict axis*. It had been
   a fifth `DecisionState`, and the owner rejected that: *"Hold is not something
   that should go in review status… It is not a review status of a retro item."*
   It became a separate flag with its own table, and the stored `hold` verdicts
   stayed where they were — see §The verdict machine.
2. **Retro 4 `r-remove-hold`** then removed *the flag itself*, feature and all:
   what the flag meant was already `involvement` — the dial that rides on the
   verdict saying how much of the work the AI may do alone, up to "not without me
   in the loop". The owner: *"I can achieve the whole thing by selecting
   something to be only done with the human in the loop."*
...
rewritten in the next revision** (retro 4 `r-verdict-revise`), which is an
...
**Latest wins, against the content it was given for** (D2, "carry-over"). A
...
**`hold` is a fifth position that no one can write.** It was a verdict until
retro 3 `r-hold-semantics` (step 1 above), and every read path still admits one —
the wire views, the export, a record's decision history, the table's own CHECK —
because human data is append-only and the owner's real store holds rows carrying
it. The **input** enum, `decisionVerdictSchema`, is the four above and refuses it.
If you see one: read it, render it, never write another. A record carrying one is
born `open` on the status axis, like everything that is not `declined`.

The same narrowing was done once before, to `solutionLevel`: values the owner cut
stayed readable forever and left the write path. Narrow the write path, never the
read path.
...
The axis that outlives the review (the owner's session-8 and session-9 asks). It
starts being interesting exactly when the verdict stops.
...
resolve carries at least one — *"we should be able to specify a commit id or
github issue or something as reference so that it is easy to see"* — and every
...
one: *"we still want to maintain its discussion"* is the whole reason the state
exists. Nothing anywhere removes a record.

**This axis is not closed by the close.** `SetRecordLifecycleUseCase` deliberately
does not call `refuseWhenFinished`, because the owner asked for it *because* the
retro is closed — *"even after a retro has been closed, we should be able to
attach metadata to issues so that we can manage their life cycle."* It endangers
...
find is a discussion nobody can read. What each state offers on the row is the
...
which is `archived`. The owner: *"for only the records that are marked as
declined during the retro and we still want to maintain its discussion, etc. so
maybe we can have a type called archived … by default all others that have
approval, those are normal records."*
...
It is flagged for the owner rather than worked around, because the alternative —
...
looking at, and the owner kept it: *"the user should be able to unarchive … if a
user wants, they can just archive it."*
...
so the next lane does not have to rediscover it:
...
- **Human only.** *"only the human should be able to mark it, not the AI"* —
  `ResolveThreadUseCase` asserts the actor as its first statement, below every
...
four deliberate exceptions**, each of which the owner asked for *because* the
retro is closed:

| Exception | Asked for | Session |
|---|---|---|
| `records.setLifecycle` | the record status axis above, for the reason that section gives | 8 |
| `labels.apply` | *"they could actually put a label that says 'migrated'"* — the move-it-to-GitHub archetype, which happens after the close by construction | 10 |
| `attributes.set` | the same archetype, carrying the ticket id beside the label | 10 |
| `records.relate` | *"both actors can relate records … so that AI can easily find past records"* — the far end of a relation is normally in a retrospective that closed sessions ago | 11 |
