...
export type AddCommentInput = {
   ...
   * never swapped in, so the reader may be pinned to revision 1 while revision
   * 2 exists, and the comment belongs to what they were reading. The CLI does
   * not send one and does not need to — the AI comments on the draft it just
   * filed — so an absent value resolves to the latest revision here, in the
