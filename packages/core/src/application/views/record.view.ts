 ...
 *
 * **There is no `hold` beside the decision any more** (`r-remove-hold`).
 * A lifecycle flag rode here for one session and was removed on first contact:
 ...
 * item without the human is `involvement`, which is already on the decision. The
 * `holds` table keeps every row that was written.
 *
 ...
 * `open`, `resolved` or `archived`, plus the entry in force
...
export type RecordViewWithLifecycle = RecordView & {
   ...
   * projection gap, one table further out. `undefined` is a record nobody
