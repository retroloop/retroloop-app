 ...
 *
 * The owner's ruling, verbatim: *"Usually labels are just labels … to keep it
 * flexible we will not hardcode any labels or attributes … adding those will
 ...
 * special in any reader: it is the example he used, not a value the product
 * ships. A store that has never had a human open the settings page holds zero
 * rows in this table and the label surfaces render nothing at all.
 *
 * **No append-only triggers, which is the departure worth naming.** Every other
 * table this session adds carries them, and so does nearly every table added in
 * the four sessions before it. This one is *configuration* rather than human
