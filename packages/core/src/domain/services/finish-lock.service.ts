 ...
 * human's `ReviewFinished`, which since retro 4 `r-one-finish-button` closes his
 * side of a round and leaves the retrospective `reviewing`. Between the two he
 ...
 * trusting the answer it got when he pressed the button.
 *
 * **Every human write path on the *review* calls this.** There were two
 * exceptions for one session — `holds.set` and `holds.clear`, which stayed
 * reachable after a finish because the solving side read the hold long after the
 * review closed (retro 3 `r-hold-semantics`) — and retro 4 `r-remove-hold` took
 * the feature out, so those went with it.
 *
 * **What has grown since is a class of write that is not on the review at all**,
 * and each one is here because the owner asked for it *because* the retro is
 * closed:
 *
 * - `records.setLifecycle` — *"even after a retro has been closed, we should be
 *   able to attach metadata to issues so that we can manage their life cycle"*
 *   (session 8).
 * - `labels.apply` and `attributes.set` — the second usage archetype, in his
 *   words: *"on completion of the retro they may actually want to move
 *   everything into GitHub right away … they could actually put a label that
 *   says 'migrated'"*. That labelling happens after the close by construction
 *   (session 10).
 * - `records.relate` — *"both actors can relate records, each relation carries
 *   how-they-relate words, and the relation reads from both sides, so that AI
 *   can easily find past records and build holistic solutions"* (session 11).
 *   It is the most closed-retro-shaped of the four: the record being related
 *   **to** is normally in a retrospective that closed sessions ago, so a lock
