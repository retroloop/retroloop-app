 ...
 * practice, and usually labels are just labels. A team that wants the detail
 * beside the classification creates an attribute and sets it, which is the
 * second primitive existing for exactly that reason.
 *
 * **Human-only, for now.** Applying a label and setting an attribute value are
 * the human's, regardless of the AI-config-write toggle — that toggle governs
 * the *definitions* (`setting.model.ts`), and whether the AI may ever suggest a
 * label on its own draft is still open. So this is a human field with
 * everything that implies: the use case refuses the `ai` actor before it reads
 * anything, the table's append-only triggers back that up at L1, and there is
 * **no `actor` column** because the table is single-writer and the author is
 * implied by it.
 *
