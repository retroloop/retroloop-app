 ...
 * may be non-`finished` at a time (KC-0011).
 *
 * State machine (D3, amended by retro 4 `r-one-finish-button`):
 ...
 *   event, not a transition: the human's one button closes *his* side of the
 *   round, and the round's outcome is what he wrote in it.
 * - `reviewing → finished` only through `ReviewClosed`, the AI's explicit close
 *   to export — which it may only take after his `ReviewFinished` for the
 ...
 * (`enums.schema.ts`). The wire restated it for four sessions and nothing would
