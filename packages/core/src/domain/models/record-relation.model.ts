 ...
 * (`record-id.model.ts`) — it is minted once and never moves, and it is what the
 * owner and the AI cite to each other. It also costs a cross-retrospective
 ...
 * words"* — the words are half the act. A relation with none is the bare link
 ...
 * vocabulary, on `record_lifecycle.refs`' argument: the owner named several kinds
 ...
 * else — *"both actors can relate records"* is the first clause of the ask — and
 ...
 * somebody entered it — never a mirror row. *"Reads from both sides"* is a
...
export type RecordRelationEntry = {
   ...
   *
   * A 0/1 column rather than a status word, and this is the fork
   * `record_labels` and `record_lifecycle` chose opposite ways on
   ...
   * to be a scale that grew from two positions to four inside one session;
